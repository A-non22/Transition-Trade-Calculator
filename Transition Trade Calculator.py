...
# ------------------- Calculator 2: Entry Calculator -------------------
with col2:
    st.header("Entry Calculator")

    mdrc = st.selectbox("MDRC (Total credit -3)", options=[0, 1, 2, 3], key="mdrc")
    long_short_structure = st.selectbox("Long/Short Structure (Total credit -2)", options=[0, 1, 2], key="long_short_structure")
    ...
    obtained_credit_2 = (
        ...
        m7box_reject + ddr_line_reject + wdrr_line_reject
    )
    ...
    st.header("Results")
    st.markdown(
        f"<h3><b>Obtained Credit:</b> <span style='color:green;'>{obtained_credit_2} / {base_total_credit_2}</span></h3>",
        unsafe_allow_html=True
    )
    st.markdown(
